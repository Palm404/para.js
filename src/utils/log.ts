...
import { parseWebhook, toTitleCase, trim } from '../utils/util';
...
export function log(
  text: any,
  type: 'log' | 'error' = 'log',
  route?: { path: string; method: Methods }
) {
...
function consoleLog(text: any, type: 'log' | 'error') {
  console[type](
    colorette.bold(
      colorette[type === 'error' ? 'red' : 'green'](
        text?.replaceAll?.(/(```(\w+)?|`|\*|__|~~)/g, '') ?? text
      )
    )
  );
}
